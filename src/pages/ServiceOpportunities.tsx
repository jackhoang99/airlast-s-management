...
const ServiceOpportunities = () => {
  ...
  useEffect(() => {
    const fetchOpportunities = async () => {
      ...
      try {
        ...
        const opportunitiesData = (jobsData || []).map(job => {
          const totalCost = job.job_items?.reduce((sum, item) => sum + Number(item.total_cost), 0) || 0;
          const priority = totalCost > 1000 ? 'high' : totalCost > 500 ? 'medium' : 'low';
        ...
        const highPriority = opportunitiesData.filter(opp => opp.priority === 'high').length;
        const estimatedRevenue = opportunitiesData.reduce((sum, opp) => sum + opp.estimatedValue, 0);
        const avgOpportunityValue = totalOpportunities > 0 ? estimatedRevenue / totalOpportunities : 0;
  ...
  const filteredOpportunities = opportunities.filter(opportunity => {
    if (!searchTerm && filters.type === 'all' && filters.priority === 'all') return true;

    const matchesSearch = !searchTerm || 
      opportunity.jobName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      opportunity.location?.companies?.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      opportunity.location?.name.toLowerCase().includes(searchTerm.toLowerCase());

    const matchesType = filters.type === 'all' || opportunity.type === filters.type;
    const matchesPriority = filters.priority === 'all' || opportunity.priority === filters.priority;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-semibold mt-1">{stats.totalOpportunities}</p>
            </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-semibold mt-1">{stats.highPriority}</p>
            </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-semibold mt-1">${stats.estimatedRevenue.toLocaleString()}</p>
            </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500">Avg Opportunity Value</p>
              <p className="text-3xl font-semibold mt-1">${Math.round(stats.avgOpportunityValue).toLocaleString()}</p>
            </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-gray-500">
            No service opportunities found. Complete more jobs to generate opportunities.
          </div>
        ) : (
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`badge ${getPriorityBadgeClass(opportunity.priority)}`}>
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      ...
                      <div className="flex items-center gap-1">
                        <Calendar size={14} />
                        <span>{new Date(opportunity.completedDate).toLocaleDateString()}</span>
                      </div>
                    </div>
