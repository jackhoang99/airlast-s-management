...
const JobDetails = () => {
  ...
  const [newItemsAfterConfirmation, setNewItemsAfterConfirmation] =
    useState(false);
  ...
  const handleSendQuote = async () => {
    ...
    try {
      ...
      // Update local state
      setJob((prev) =>
        prev ? { ...prev, quote_sent: true, quote_token: quoteToken } : null
      );
  ...
  const handleSendUpdatedQuote = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Failed to send updated quote email"
        );
      }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to send updated quote"
      );
    } finally {
      setIsSendingQuote(false);
    }
  ...
  const handleCreateInvoice = async () => {
    ...
    try {
      ...
      const invoiceNumber = `${job.number}-INV-${Math.floor(
        Math.random() * 10000
      )
        .toString()
        .padStart(4, "0")}`;
          ...
          <div className="flex items-center">
            ...
            <div>
              <h3 className="font-medium text-success-800">
                Quote Confirmed by Customer
              </h3>
              <p className="text-success-700">
                Confirmed on{" "}
                {job.quote_confirmed_at
                  ? new Date(job.quote_confirmed_at).toLocaleDateString()
                  : "N/A"}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-start">
            ...
            <div className="flex-1">
              <h3 className="font-medium text-warning-800">
                New items added after quote confirmation
              </h3>
              <p className="text-warning-700 mb-4">
                Items have been added to this job after the customer confirmed
                the quote. Consider sending an updated quote to the customer.
              </p>
              <button
        ...
        <div className="bg-success-50  border border-success-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-success-700">
                The quote has been sent to {job.contact_email}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-success-50 border border-success-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-success-700">
                The invoice has been sent to {job.contact_email}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getContractBadgeClass(
                      isContractJob(job)
                    )}`}
                  >
                  ...
                  {job.quote_sent && (
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getQuoteBadgeClass(
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {job.contact_type || "Primary"} Contact
                    </p>
                  </div>
                </div>
                    ...
                    {jobItems.map((item, index) => {
                      ...
                      const isNewItem =
                        ...
                        new Date(item.created_at) >
                          new Date(job.quote_confirmed_at);
                  ...
                  <tbody>
                    {jobInvoices.map((invoice, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm">
                          <span
                            className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getInvoiceStatusBadgeClass(
                              invoice.status
                            )}`}
                          >
                            {invoice.status}
