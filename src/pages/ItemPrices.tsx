...
import { ArrowLeft, Plus, Edit, Trash2, Search, Filter, Package, PenTool as Tool, ShoppingCart, Check, X, AlertTriangle } from 'lucide-react';

type JobItemPrice = Database['public']['Tables']['job_item_prices']['Row'];

const ItemPrices = () => {
  const { supabase } = useSupabase();
  const [items, setItems] = useState<JobItemPrice[]>([]);
  const [filteredItems, setFilteredItems] = useState<JobItemPrice[]>([]);
  ...
  const openEditModal = (item: JobItemPrice) => {
  ...
  const openDeleteModal = (item: JobItemPrice) => {
    setSelectedItem(item);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-semibold">Add Item Price</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={newItem.type}
                  onChange={(e) => setNewItem(prev => ({ ...prev, type: e.target.value as 'part' | 'labor' | 'item' }))}
                  className="select"
                  required
                >
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                className="btn btn-primary"
                disabled={!newItem.code || !newItem.name || newItem.unit_cost <= 0}
              >
                <Plus size={16} className="mr-2" />
                Add Pricing
              </button>
