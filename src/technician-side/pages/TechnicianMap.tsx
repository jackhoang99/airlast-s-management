...
import { MapPin, Calendar, Clock, Navigation, List, Search, ArrowLeft, Briefcase, AlertTriangle, Phone, Mail, Layers, Car, RotateCw, Compass, Maximize, Minimize, User, ChevronDown, ChevronUp, X } from 'lucide-react';
...
const TechnicianMap = () => {
  ...
  const [directionsSteps, setDirectionsSteps] = useState<google.maps.DirectionsStep[]>([]);
    ...
    if (navigator.geolocation) {
      ...
      const watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          if (isNavigating && map && directionsService && directionsRenderer && selectedJobId) {
            const selectedJob = jobs.find(job => job.id === selectedJobId);
            if (selectedJob && selectedJob.locations?.lat && selectedJob.locations?.lng) {
  ...
  useEffect(() => {
    const initMap = async () => {
      try {
        ...
        if (mapRef.current && technicianLocation) {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        ...
        const jobsWithCoordinates = await Promise.all(jobsData.map(async (job) => {
          if (job.locations) {
  ...
  useEffect(() => {
    if (map && jobs.length > 0 && selectedJobId) {
      const selectedJob = jobs.find(job => job.id === selectedJobId);
      if (selectedJob && selectedJob.locations?.lat && selectedJob.locations?.lng) {
        map.setCenter({ lat: selectedJob.locations.lat, lng: selectedJob.locations.lng });
        map.setZoom(15);
  ...
  }, [selectedJobId, map, jobs, technicianLocation, directionsService, directionsRenderer]);
    ...
    jobsToMark.forEach(job => {
      if (job.locations?.lat && job.locations?.lng) {
        ...
        const marker = new google.maps.Marker({
          ...
          icon: {
            path: google.maps.SymbolPath.CIRCLE,
            fillColor: job.id === selectedJobId ? '#ef4444' : getJobStatusColor(job.status),
            fillOpacity: 1,
            strokeWeight: 1,
            strokeColor: '#FFFFFF',
        ...
        const infoWindow = new google.maps.InfoWindow({
          content: `
            <div style="max-width: 200px;">
              ...
              <a href="/tech/jobs/${job.id}" style="color: #0672be; font-size: 12px; text-decoration: none;">View Job Details</a>
            </div>
          `
        });
        ...
        if (status === google.maps.DirectionsStatus.OK && result) {
          ...
          if (route && route.legs.length > 0) {
            ...
            if (route.legs[0].duration) {
              const now = new Date();
              const arrivalDate = new Date(now.getTime() + (route.legs[0].duration.value * 1000));
  ...
  const handleNavigateToJob = (job: any) => {
    if (!technicianLocation || !job.locations?.lat || !job.locations?.lng) return;
    
    calculateRoute(
      technicianLocation,
      { lat: job.locations.lat, lng: job.locations.lng }
    );
  };
  ...
  return (
    <div className={`h-full flex flex-col ${isFullscreen ? 'fullscreen' : ''}`}>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        {/* Map */}
        <div className={`flex-1 bg-gray-100 relative ${showJobsList ? 'hidden md:block' : ''}`}>
          {error ? (
          ...
          <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
            ...
            <button 
              ...
              className={`p-3 rounded-full shadow-md ${trafficEnabled ? 'bg-primary-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
                  ...
                  {directionsSteps.map((step, index) => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className={`${index === currentStepIndex ? 'font-medium' : ''}`}>
                ...
                {filteredJobs.map(job => (
                  <div 
                    ...
                    className={`p-4 hover:bg-gray-50 cursor-pointer ${
                      selectedJobId === job.id ? 'bg-primary-50 border-l-4 border-primary-500' : ''
                    }`}
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="text-right">
                        <div className="flex items-center text-xs text-gray-600">
                          ...
                          {job.schedule_start ? formatDateTime(job.schedule_start).split(',')[0] : 'Unscheduled'}
                        </div>
                        {job.schedule_start && (
                ...
                {isNavigating && estimatedTime && estimatedDistance && (
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-lg font-bold">{estimatedDistance}</div>
                    </div>
                  </div>
                )}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="font-medium">
                          {formatInstructions(directionsSteps[currentStepIndex].instructions)}
                        </div>
                      </div>
