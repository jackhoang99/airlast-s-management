...
import { Calendar, ChevronLeft, ChevronRight, Clock, MapPin, AlertTriangle, CheckSquare, Briefcase } from 'lucide-react';
  ...
  if (view === 'week') {
    ...
    jobs.forEach(job => {
      if (job.schedule_start) {
        const jobDate = new Date(job.schedule_start).toISOString().split('T')[0];
            ...
            <div className="absolute top-0 left-0 w-16 h-full border-r border-gray-200">
              {timeSlots.map(hour => (
                <div key={hour} className="h-20 border-b border-gray-100 flex items-start justify-center pt-1">
                  <span className="text-xs text-gray-500">{hour === 0 ? '12 AM' : hour < 12 ? `${hour} AM` : hour === 12 ? '12 PM' : `${hour-12} PM`}</span>
                </div>
              ))}
            </div>
            ...
            <div className="ml-16 relative">
              ...
              {jobs.map(job => {
                ...
                const duration = job.schedule_duration ? 
                  parseFloat(job.schedule_duration.replace(' hours', '')) : 1;
                ...
                return (
                  <Link 
                    ...
                    className={`absolute left-2 right-2 p-2 rounded-md overflow-hidden ${
                      ...
                      job.status === 'cancelled' ? 'bg-red-100 border border-red-200' :
                  ...
                  >
                    ...
                    <div className="text-xs truncate">{job.locations?.name}</div>
