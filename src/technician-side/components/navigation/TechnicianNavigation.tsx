...
import { X, Navigation, Clock, MapPin, Phone, Mail, ArrowLeft, ChevronRight, Info, Compass, Car, RotateCw, Layers, Maximize, Minimize, User, AlertTriangle, Check } from 'lucide-react';
...
const TechnicianNavigation = ({ isOpen, onClose, location, job }: TechnicianNavigationProps) => {
    ...
    if (navigator.geolocation) {
      ...
      const watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          if (isNavigating && map && directionsService && directionsRenderer && destinationLocation) {
  ...
  }, [isOpen, isNavigating, map, directionsService, directionsRenderer, destinationLocation]);
    ...
    const initMap = async () => {
      try {
        ...
        if (mapRef.current && technicianLocation && destinationLocation) {
          ...
          // Calculate route
          calculateRoute(technicianLocation, destinationLocation, directionsServiceInstance, directionsRendererInstance, google.maps);
  ...
  }, [isOpen, technicianLocation, destinationLocation, mapType, trafficEnabled]);
      ...
      <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-white shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`p-2 rounded-full ${showJobInfo ? 'bg-primary-100 text-primary-600' : 'hover:bg-gray-100 text-gray-500'}`}
            title="Job Info"
          >
            <Info size={20} />
          </button>
          ...
          <div className="p-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                {job.units && <p className="text-sm">Unit: {job.units.unit_number}</p>}
              </div>
                ...
                <div>
                  ...
                  {job.contact_phone && (
                    <div className="flex items-center gap-1 text-sm">
                      ...
                      <a href={`tel:${job.contact_phone}`} className="text-primary-600">{job.contact_phone}</a>
                    </div>
                  )}
                  {job.contact_email && (
                    <div className="flex items-center gap-1 text-sm">
                      <Mail size={14} className="text-gray-400" />
                      <a href={`mailto:${job.contact_email}`} className="text-primary-600">{job.contact_email}</a>
                    </div>
                  )}
                </div>
              )}
