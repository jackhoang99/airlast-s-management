...
const AddJobPricingModal = ({
  ...
  const [selectedTab, setSelectedTab] = useState<"part" | "labor" | "item">(
    "part"
  );
  ...
  // Selected item
  const [selectedPartItem, setSelectedPartItem] = useState<JobPartPrice | null>(
    null
  );
  ...
  useEffect(() => {
    if (selectedTab === "part") {
      const roofFee = includeRoofAccessFee
        ? Number(newItem.roof_access_fee)
        : 0;
      ...
      const totalBaseCost =
        ...
        Number(newItem.estimated_hours) *
          Number(newItem.complexity_multiplier) *
          adjustedLaborCost +
      ...
      if (editItem) {
        ...
        if (jobId) {
          const finalUnitCost =
            selectedTab === "part"
              ? selectedPricingType === "non-contract"
                ? newItem.flat_rate_non_contract
                : newItem.flat_rate_pm_contract
              : newItem.unit_cost;
        ...
        // Only add to price tables if no jobId is provided (we're in the pricing management section)
          ...
          const { error } = await supabase.from("job_part_prices").insert({
            ...
            adjusted_labor_cost: multipleTechs
              ? newItem.adjusted_labor_cost + 50
              : newItem.adjusted_labor_cost,
                    ...
                    {filteredPartItems.map((item) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-500 flex justify-between">
                          <span>
                            Parts: ${Number(item.parts_cost).toFixed(2)} |
                            Hours: {Number(item.estimated_hours).toFixed(1)}
                          </span>
                        </div>
                        <div className="text-xs text-gray-500 flex justify-between">
                          <span>
                            Non-Contract: $
                            {Number(item.flat_rate_non_contract).toFixed(2)}
                          </span>
                          <span>
                            PM-Contract: $
                            {Number(item.flat_rate_pm_contract).toFixed(2)}
                          </span>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="flex items-center">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setIncludeRoofAccessFee(!includeRoofAccessFee)
                        }
                        className={`relative inline-flex h-5 w-10 items-center rounded-full ${
                          includeRoofAccessFee
                            ? "bg-primary-600"
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <span className="text-lg font-semibold">
                          ${newItem.flat_rate_non_contract.toFixed(2)}
                        </span>
                      </div>
                    </div>
                    {selectedPricingType === "non-contract" && (
                ...
                >
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <div className="relative">
                        <span className="text-lg font-semibold">
                          ${newItem.flat_rate_pm_contract.toFixed(2)}
                        </span>
                      </div>
                    </div>
                    {selectedPricingType === "pm-contract" && (
