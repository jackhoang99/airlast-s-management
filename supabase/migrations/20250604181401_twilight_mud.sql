/*
  # Fix Inspection Quote Template Default

  1. Changes
    - Creates a function to copy an existing template to a new template type
    - Copies the existing repair template to create an inspection template
    - Sets the new inspection template as default
*/
...
DECLARE
  ...
  new_inspection_template_id UUID;
BEGIN
  -- Find the default repair template
  ...
  -- If a default repair template exists, copy it to create an inspection template
  IF repair_template_id IS NOT NULL THEN
    -- First, unset any existing default inspection templates
    UPDATE quote_templates
    SET template_data = jsonb_set(template_data, '{isDefault}', 'false')
    WHERE template_data->>'templateType' = 'inspection'
    AND template_data->>'isDefault' = 'true';
    
    -- Copy the repair template to create an inspection template
    SELECT copy_template_to_new_type(
      repair_template_id,
      'Default Inspection Template',
      'inspection'
    ) INTO new_inspection_template_id;
    
    RAISE NOTICE 'Created new inspection template with ID: %', new_inspection_template_id;
  ELSE
