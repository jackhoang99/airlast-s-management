/*
  # Customer Portal Tables and Functions

  1. New Tables
    - `customer_portal_users` - Stores users for the customer portal
    - `customer_portal_sessions` - Stores session information for customer portal users
  ...
  3. Functions
    - Add function to verify customer portal passwords
*/
...
-- Fixed: Removed reference to non-existent column u.company_id
...
-- Fixed: Changed the policy to use authenticated role instead of specific user check
CREATE POLICY "Users can view their own sessions" 
  ON customer_portal_sessions
  FOR SELECT
  TO authenticated
  USING (user_id = customer_portal_users.id);
