...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: quoteData, error: quoteError } = await supabase.from('job_quotes').select('*').eq('token', token).maybeSingle();
    if (quoteError && !quoteError.message.includes("contains 0 rows")) {
      throw quoteError;
    }
    // If quote record exists, update it
    if (quoteData) {
      const { error: updateQuoteError } = await supabase.from('job_quotes').update({
        confirmed: true,
      ...
      const { error: updateJobError } = await supabase.from('jobs').update({
    ...
    const { data: jobData, error: jobError } = await supabase.from('jobs').select('*').eq('quote_token', token).maybeSingle();
    if (jobError) {
      throw jobError;
    }
    if (!jobData) {
