...
serve(async (req)=>{
  ...
  try {
    ...
    const preservedPages = templateData.template_data.preservedPages || [
      1
    ];
    ...
    const { error: uploadError } = await supabase.storage.from('quotes').upload(pdfPath, pdfBytes, {
    ...
    const { data: urlData, error: urlError } = await supabase.storage.from('quotes').createSignedUrl(pdfPath, 3600); // 1 hour expiry
    if (urlError) {
      throw new Error(`Error creating signed URL: ${urlError.message}`);
    }
    // 12. Create a record in the job_quotes table if it doesn't exist
    const { data: existingQuote, error: existingQuoteError } = await supabase.from('job_quotes').select('*').eq('job_id', jobId).eq('quote_type', quoteType).eq('quote_number', quoteNumber).maybeSingle();
    ...
    if (!existingQuote) {
      ...
      const { error: insertError } = await supabase.from('job_quotes').insert({
        job_id: jobId,
