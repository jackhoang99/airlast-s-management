...
serve(async (req) => {
  ...
  try {
    ...
    const { jobId, quoteType, quoteNumber, templateId, jobData, inspectionData, replacementData, jobItems, replacementDataById } = await req.json();
    ...
    console.log("Received data:", {
      ...
      hasReplacementDataById: !!replacementDataById && Object.keys(replacementDataById || {}).length > 0
    });
    ...
    if (quoteType === 'replacement') {
      ...
      if (replacementDataById && Object.keys(replacementDataById).length > 0) {
        console.log("Using replacementDataById with keys:", Object.keys(replacementDataById));
        
        // Convert the object to an array for processing
        replacementsToProcess = Object.entries(replacementDataById).map(([key, data], index) => ({
          ...data,
          replacementNumber: index + 1,
          created_at: data.created_at || new Date().toISOString()
        }));
      }
      // If we have an array of replacement data, use it
      else if (Array.isArray(replacementData) && replacementData.length > 0) {
        console.log("Using array replacementData with length:", replacementData.length);
      ...
      // If we have a single replacement data object, use it
      ...
      // If no replacement data provided, fetch it from the database
      else {
        ...
        if (!fetchError && fetchedReplacements && fetchedReplacements.length > 0) {
    ...
    // For repair quotes, use job items
    ...
    if (quoteType === 'replacement' && replacementsToProcess.length > 0) {
      console.log(`Processing ${replacementsToProcess.length} replacement options`);
      ...
      for (let i = 0; i < replacementsToProcess.length; i++) {
        ...
        dynamicPage.drawText(`Replacement ${entry.replacementNumber} (${repairDate})`, {
          x: margin,
          y,
          size: fontSize,
          font: bold
        ...
        const additionalItems = entry.additionalItems || entry.additional_items || [];
        ...
        const rowsUsed = Math.ceil(components.length / Math.max(1, Math.floor((width - 2 * margin) / columnWidth)));
    ...
    // For repair quotes, show job items
    else if (quoteType === 'repair' && Array.isArray(jobItems) && jobItems.length > 0) {
