...
serve(async (req) => {
  ...
  try {
    ...
    if (repairDataByInspection && typeof repairDataByInspection === 'object' && Object.keys(repairDataByInspection).length > 0) {
      ...
      console.log(`Processing ${repairsToProcess.length} repairs from repairDataByInspection`);
      ...
      console.log(`Processing ${repairsToProcess.length} repairs from repairData array`);
      ...
      console.log("Processing single repair from repairData object");
    }
    
    console.log(`Total repairs to process: ${repairsToProcess.length}`);
    
    if (repairsToProcess.length > 0) {
      // Check if we need a new page
      if (y < 200) { // Need more space for repair summary
        dynamicPage = newPdfDoc.addPage();
        y = height - margin;
      ...
      for (let i = 0; i < repairsToProcess.length; i++) {
        ...
        // Calculate position for right-aligned cost
        const costWidth = bold.widthOfTextAtSize(formattedCost, fontSize);
        
        dynamicPage.drawText(formattedCost, {
          x: width - margin - costWidth,
          y,
          size: fontSize,
          font: bold
        });
      ...
      if (repairsToProcess.length > 1) {
        ...
        // Format the combined total with commas
        const formattedTotal = combinedTotal.toLocaleString('en-US', {
          style: 'currency',
          currency: 'USD',
          minimumFractionDigits: 0,
          maximumFractionDigits: 0
        ...
        // Calculate position for right-aligned total
        const totalWidth = bold.widthOfTextAtSize(formattedTotal, fontSize + 2);
        
        dynamicPage.drawText(formattedTotal, {
