...
serve(async (req) => {
  ...
  try {
    ...
    // Get request body
    const { jobId, customerEmail, quoteToken, jobNumber, jobName, customerName, totalAmount } = await req.json();
    ...
    const msg = {
      to: customerEmail,
      from: "quotes@airlast.com", // Replace with your verified sender
